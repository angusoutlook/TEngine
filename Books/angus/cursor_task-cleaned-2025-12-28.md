...
Nice，现在关键拼图齐了：`FTask/FTask<T>` 是“可 await 的一次性完成信号 + 结果/异常载体”，`Builder/*` 把 C# `async` 状态机直接绑定到 `FTask` 上，`FTaskCompleted/FVoid` 用来分别承载“同步已完成”和“fire-and-forget 协程启动”。我下面会按“架构组件→执行流→示例→对比 System.Task”的顺序讲清楚设计意图。我使用的 AI 模型是 **GPT-5.2**。
...
```markdown
*** Begin Patch
*** Update File: books/angus/FTask-Async-StateMachine-Conversation-Transcript-2025-12-28.md
@@
-# FTask & Async State Machine — Conversation Transcript (2025-12-28)
-
-> Note: This file is a verbatim transcript of the conversation, preserved "as-is" (no omissions).
...
-- **`FVoid`**：一个“只为启动 async 状态机/火忘流程”的壳；异常策略在 builder 中定义（直接 throw），并经常配合 `.Coroutine()` 风格启动。
-
-如果你希望我再把 `FVoid`/`FTaskCompleted`/`FTask` 的“哪些成员是为了满足 await 语法、哪些成员是为了池化/异常传播”画成一张对照表，我也可以继续补上。
-```
-
-## User
-
...
+- 取 `Bar()` 返回的 awaiter（你们这里通常就是 `FTask` 自己，`GetAwaiter()=>this`）
+- 如果 `awaiter.IsCompleted == true`：直接继续执行后面的代码（同步穿透）
+- 如果 `IsCompleted == false`：记录 `_state`，把“继续执行 Foo 的入口”注册给 awaiter，然后返回（Foo 挂起）
+
+注册动作在你们 builder 里是：
+
+- `awaiter.UnsafeOnCompleted(stateMachine.MoveNext)`
